...
// import { testGrid } from "./map.spec.data";
...
//         [L,L]
//     ]);
//     const result = getConectedHexGroupsByType(grid).map(e => e.sort(sortHexArray)).sort(sort2DHexArray);
...
// test('getConectedHexGroupsByType: large test', () => {
//     var grid = testGrid;
//     const expected = [
//         [grid.hexes[0][0], grid.hexes[0][1], grid.hexes[1][0]],
//         grid.hexes.flat().filter(h => h.type === HexType.Water), 
...
//         const grid = new Grid();
//         createRandomMap(grid);
//         grids.push(grid);
//     }
//     const aveNumSeparateLands = grids.reduce((acc, grid) => {
//         const numSeparateLands = getConectedHexGroupsByType(grid).filter(group => group[0].type === HexType.Land).length;
//         return acc + numSeparateLands;
//     }, 0) / iterations;
// });
